...
> [!NOTE]
> Żeby przekazać `CancellationToken` do sekwencji można użyć metody roszerzającej `WithCancellation(CancellationToken token)`, np: `await foreach(var item in asyncSequence.WithCancellation(token))`.
